...
import { fetchProjectDetails } from "../features/workspaceSlice";
...
export default function ProjectDetail() {
    ...
    const isAdmin = useSelector((state) => {
        const members = state.workspace.currentWorkspace?.members;
    ...
    useEffect(() => {
        if (id) dispatch(fetchProjectDetails(id));
    }, [id, dispatch]);
            ...
            <div>
                <div className="inline-flex flex-wrap max-sm:grid grid-cols-3 gap-2 border border-zinc-200 dark:border-zinc-800 rounded overflow-hidden">
                    {[
                        ...
                        <button key={tabItem.key} onClick={() => { setActiveTab(tabItem.key); setSearchParams({ id, tab: tabItem.key }); }} className={`flex items-center gap-2 px-4 py-2 text-sm transition-all ${activeTab === tabItem.key ? "bg-zinc-100 dark:bg-zinc-800/80" : "hover:bg-zinc-50 dark:hover:bg-zinc-700"}`}>
                ...
                <div className="mt-6">
                    {activeTab === "tasks" && <div className="dark:bg-zinc-900/40 rounded max-w-6xl"><ProjectTasks tasks={tasks} /></div>}
