...
function WorkspaceDropdown() {
    ...
    // Fetch projects for initial workspace
    useEffect(() => {
        ...
        <div className="relative m-4" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute z-50 w-64 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded shadow-lg top-full left-0">
                    <div className="p-2 overflow-y-auto max-h-60 no-scrollbar">
                        <p className="text-xs text-gray-500 dark:text-zinc-400 uppercase tracking-wider mb-2 px-2">
                            Workspaces

                        </p>
                        {workspaces.map((ws) => (
                            <div key={ws._id} onClick={() => onSelectWorkspace(ws)} className="flex items-center gap-3 p-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-zinc-800" >
                                {ws.image_url ? (
            ...
            <CreateWorkspaceDialog isOpen={isCreateOpen} setIsOpen={setIsCreateOpen} />
        </div>
    );
}
