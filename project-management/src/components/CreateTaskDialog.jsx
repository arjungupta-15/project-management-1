...
export default function CreateTaskDialog({ showCreateTask, setShowCreateTask, projectId }) {
    const { projects, currentWorkspace } = useSelector((state) => state.workspace);
    const project = projects.find((p) => p._id === projectId);
    // Use workspace members so all invited members show up as assignee options
    const teamMembers = currentWorkspace?.members?.filter(m => m.user?.name || m.user?.email).map(m => m.user) || [];
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await dispatch(addTask({ ...formData, projectId })).unwrap();
