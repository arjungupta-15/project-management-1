...
function MyTasksSidebar() {
    ...
    useEffect(() => {
        if (!user?._id || !projects?.length) return;
        const tasks = projects.flatMap((project) =>
            (project.tasks || [])
                ...
                .map((task) => ({ ...task, projectId: task.projectId || project._id }))
        );
        setMyTasks(tasks);
    }, [projects, user]);
                ...
                <div className="mt-2 pl-2">
                    <div className="space-y-1">
                        {myTasks.length === 0 ? (
                            ...
                            myTasks.map((task) => (
                                <Link key={task._id} to={`/taskDetails?projectId=${task.projectId}&taskId=${task._id}`} className="w-full rounded-lg transition-all duration-200 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-black dark:hover:text-white" >
